"""
Custom attention processor for HuggingFace Mistral that routes attention to
...
class MetalLLMMistralAttnProcessor(nn.Module):
    ...
    def forward(
        ...
        if self.profiler is not None:
            self.profiler.mark("mistral_attention", t0)
...
def install_mistral_attention_processor(model: nn.Module, context_manager: ContextManager, profiler: Optional[Profiler] = None) -> int:
    replaced = 0
    proc = MetalLLMMistralAttnProcessor(context_manager=context_manager, profiler=profiler)
